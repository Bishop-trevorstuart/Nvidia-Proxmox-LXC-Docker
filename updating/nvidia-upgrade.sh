...
SKIP_WORKLOAD_RESTART="${SKIP_WORKLOAD_RESTART:-false}"
...
check_proxmox() {
  if ! systemctl is-active --quiet pve; then
    error "Proxmox VE is not running"
  fi
...
get_latest_production_version() {
  ...
  local versions
  versions=$(curl -s --max-time 10 "${NVIDIA_API_URL}/" 2>/dev/null | grep -oP 'href="\K\d+\.\d+\.\d+' | sort -V | tail -10)
  if [[ -z "$versions" ]]; then
...
upgrade_lxc_driver() {
  ...
  pct exec "$vmid" -- bash -c "if command -v docker &>/dev/null; then docker stop \$(docker ps -q) 2>/dev/null || true; sleep 2; fi" || warn "Docker stop had issues or not running"
  log "Step 2/5: Uninstalling old driver in container..."
  pct exec "$vmid" -- bash -c "if command -v nvidia-installer &>/dev/null; then nvidia-installer --uninstall --silent --no-questions || true; fi" || warn "Previous driver uninstall had issues"
  ...
  pct exec "$vmid" -- bash -c "chmod +x /tmp/NVIDIA-Linux-x86_64-${NEW_VERSION}.run && /tmp/NVIDIA-Linux-x86_64-${NEW_VERSION}.run --silent --no-kernel-module --no-questions --no-x-check --no-search-path --accept-license" || error "Container driver installation failed"
  sleep 3
...
restart_container_workloads() {
  ...
  for vmid in $LXC_CONTAINERS; do
    ...
    pct exec "$vmid" -- bash -c "if command -v docker &>/dev/null; then systemctl restart docker 2>/dev/null || true; sleep 3; for compose_file in \$(find /opt /home /root -name 'docker-compose.yml' -o -name 'compose.yml' 2>/dev/null); do compose_dir=\$(dirname \"\$compose_file\"); echo \"Restarting stack in: \$compose_dir\"; cd \"\$compose_dir\" && (docker compose up -d 2>/dev/null || docker-compose up -d 2>/dev/null) || true; done; fi" || warn "Some workload restart steps failed in container $vmid"
